...
public class SnakeMoveLeft implements SnakeMove {
	...
	public ArrayList<ArrayList<Integer>> dummyMove(ArrayList<ArrayList<Integer>> snake, int length) {
		...
		for (int i = length; i >= 0; i--) {
			...
			if (result.get(i).get(SConstant.SC_COORDINATE_XPOS_KEY) < SConstant.SC_GAMEBODY_BORDER_BEGIN_XPOS) {
				result.get(i).set(SConstant.SC_COORDINATE_XPOS_KEY, SConstant.SC_SNAKE_AVAIL_MAX_XPOS);
			}
		}
		return result;
	}

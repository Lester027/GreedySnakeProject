...
public class SnakeMoveDown implements SnakeMove {
	...
	public void move(Snake snake) {
		...
		for (int i = snake.getLength(); i >= 0; i--) {
			...
			if(snake.getYCoordinate(i) > SConstant.SC_SNAKE_AVAIL_MAX_YPOS) {
				snake.setYCoordinate(i, SConstant.SC_GAMEBODY_BACKGROUND_BEGIN_YPOS);
			}
		}
	}
	...
	public ArrayList<ArrayList<Integer>> dummyMove(ArrayList<ArrayList<Integer>> snake, int length) {
		...
		for(int i = length; i >= 0; i--) {
			...
			if(result.get(i).get(SConstant.SC_COORDINATE_YPOS_KEY) > SConstant.SC_SNAKE_AVAIL_MAX_YPOS) {
				result.get(i).set(SConstant.SC_COORDINATE_YPOS_KEY, SConstant.SC_GAMEBODY_BACKGROUND_BEGIN_YPOS);
			}
		}
		return result;
	}
